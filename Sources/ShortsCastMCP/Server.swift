...
public extension ShortsCastMCP {
    ...
    static func allTools(handlers h: Handlers) -> [MCPTool] {
        ...
        return [
            ...
            MCPTool(name: "list_displays",
                    description: "List attached displays with the index to pass as `display` to start_recording, plus resolution and which is main.",
                    inputSchema: obj([:])) { await h.listDisplays($0) },
            MCPTool(name: "list_windows",
                    description: "List on-screen app windows. Each window's `target` (window number as a string) or app name can be passed as `target` to start_recording.",
                    inputSchema: obj([:])) { await h.listWindows($0) },
